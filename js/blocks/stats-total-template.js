...
const statsTotalTemplate = (state) => {
  let fast_counter = state.answers.filter(e => e === `fast`).length;
  let slow_counter = state.answers.filter(e => e === `slow`).length;
  let correct_counter = state.answers.filter(e => e === `correct`).length;
  let answers_correct_total_counter =  correct_counter + fast_counter + slow_counter;
  let lives_counter = state.lives;
  let totalScores = countScores(state);
  if( totalScores === -1 ){
    totalScores = `FAIL`;
  }
...
export default statsTotalTemplate;
